...
class PlanningGraph():
    ...
    def add_literal_level(self, level):
        ...
        # for every a node in the previous level
        for a_node in self.a_levels[previous_level]:
            # has list of effnodes that represent the effect of an action
            for s_node in a_node.effnodes:
                # make these literals a part of the new S level
                new_node.add(s_node)
                s_node.parents.add(a_node)
                a_node.children.add(s_node)
        # add new node
        self.s_levels.append(new_node)
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # all add & remove effects
        effect_add = node_a1.action.effect_add
        effect_rem = node_a2.action.effect_rem

        # check if add effects are inconsistent with remove effects
        return True if [e for e in effect_add if e in effect_rem] else False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # all effects & preconditions
        eff_add_1 = node_a1.action.effect_add
        eff_rem_1 = node_a1.action.effect_rem
        pre_pos_1 = node_a1.action.precond_pos
        pre_neg_1 = node_a1.action.precond_neg
        eff_add_2 = node_a2.action.effect_add
        eff_rem_2 = node_a2.action.effect_rem
        pre_pos_2 = node_a2.action.precond_pos
        pre_neg_2 = node_a2.action.precond_neg

        # check for mutual exclusion
        return True if [e for e in eff_add_1 if e in pre_pos_2] \
                else False or \
               True if [e for e in eff_rem_1 if e in pre_neg_2] \
                else False or \
               True if [e for e in eff_add_2 if e in pre_pos_1] \
                else False or \
               True if [e for e in eff_rem_2 if e in pre_neg_1] \
                else False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        # parent nodes
        a1_parents = node_a1.parents
        a2_parents = node_a2.parents

        # check for mutual exclusion if is mutex
        return True if [[a1, a2] for a1 in a1_parents for a2 in a2_parents \
                    if a1.is_mutex(a2)] \
                     else False
    ...
    def inconsistent_support_mutex(self, node_s1: PgNode_s, node_s2: PgNode_s):
        ...
        """
        # parent nodes
        s1_parents = node_s1.parents
        s2_parents = node_s2.parents

        # check for mutual exclusion if is mutex
        return False if [[s1, s2] for s1 in s1_parents for s2 in s2_parents \
